defmodule BotEngine.Utils.UtilsTest do
  use ExUnit.Case, async: false
  doctest BotEngine.Utils.Utils

  alias BotEngine.Utils.Utils

  test "it returns an empty string when list is empty" do
    values = []
    result = Utils.to_sentence(values)
  ...
  test "it uses custom connectors when provided" do
    single_value_result = Utils.to_sentence([1], custom_sentence_connectors)
    two_values_result = Utils.to_sentence([1, 2], custom_sentence_connectors)
    multiple_values_result = Utils.to_sentence([1, 2, 3], custom_sentence_connectors)
